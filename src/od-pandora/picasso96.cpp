 ...
 */
static void do_fillrect(uae_u8 *src,
	int x,
	int y,
	int width,
	int height,
 ...
 * srcp.
 */
static void do_blit(struct RenderInfo *ri,
	int Bpp,
	int srcx,
	int srcy,
	int dstx,
...
void InitPicasso96(void)
{
	...
	if (first_time) {
		...
		for (i = 0; i < mode_count; i++) {
			sprintf(DisplayModes[i].name,
				"%dx%d, %d-bit, %d Hz",
				DisplayModes[i].res.width,
				DisplayModes[i].res.height,
				DisplayModes[i].depth * 8,
				DisplayModes[i].refresh);
